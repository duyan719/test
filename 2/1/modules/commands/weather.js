...
module.exports.run = async ({ api, event, args }) => {
	...
	return request(encodeURI("api.openweathermap.org/data/2.5/weather?q={city name}&appid={API key}" + city + "&appid=" + global.configModule[this.config.name].OPEN_WEATHER + "&units=metric&lang=vi"), (err, response, body) => {
		if (err) throw err;
